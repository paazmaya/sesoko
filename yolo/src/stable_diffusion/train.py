...
def train_lora(
    ...
    is_local_safetensors = (
        base_model_path.exists() and base_model_path.suffix == ".safetensors"
    )
    ...
    if is_local_safetensors:
        ...
        if text_encoder_keys:
            ...
            logger.warning(
                "No text encoder weights found in local file, using random initialization"
            )
    else:
    ...
    if is_local_safetensors or "xl" in model_source.lower():
        ...
        if is_local_safetensors:
            ...
            te2_keys = [
                k
                for k in full_state_dict.keys()
                if "text_encoder_2" in k or "conditioner" in k
            ]
            if te2_keys:
                te2_state_dict = {}
                for k in te2_keys:
                    new_key = k.replace("text_encoder_2.", "").replace(
                        "conditioner.", ""
                    )
                ...
                logger.warning(
                    "No text_encoder_2 weights found in local file, using random initialization"
                )
        else:
    ...
    if is_local_safetensors:
        ...
        # Download only the config (small JSON file)
        config = AutoencoderKL.load_config(component_repo, subfolder="vae")
        # Initialize model from config without weights
        vae = AutoencoderKL.from_config(config)

        # Extract VAE weights from state dict
        vae_keys = [
            k for k in full_state_dict.keys() if "vae" in k or "first_stage_model" in k
        ]
        if vae_keys:
            # Map keys from checkpoint format to HF format
            vae_state_dict = {}
            ...
            logger.warning(
                "No VAE weights found in local file, using random initialization"
            )
    else:
    ...
    if is_local_safetensors:
        ...
        # Download only the config (small JSON file)
        config = UNet2DConditionModel.load_config(component_repo, subfolder="unet")
        # Initialize model from config without weights
        ...
        unet_keys = [
            ...
            if any(
                prefix in k
                for prefix in ["model.diffusion_model", "unet", "model.model"]
            )
        ]
        ...
        if unet_keys:
            ...
            logger.warning(
                "No UNet-specific keys found, attempting to load entire state dict as UNet"
            )
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if (
                    ...
                    and hasattr(unet.config, "addition_embed_type")
                    and unet.config.addition_embed_type == "text_time"
                ):
